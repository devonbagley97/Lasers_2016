...
public class Backtracker {
    ...
    // the path of configurations to the solution
    private  List<Configuration> path = new ArrayList<Configuration>();
    ...
    public List<Configuration> solveWithPath(Configuration current) {
        ...
        if(current.isGoal()){  // if the current config is goal add to path
            path.add(current);
            return path;
        }
        else{
            for(Configuration child : current.getSuccessors()){
                if(child.isValid()){
                    ...
                    if(sol != null){        // if the current config is part of the solution add to path
                        path.add(current);
                        return path;
